...
const ProveedorEdit = () => {
  ...
  const loadProveedor = async () => {
    try {
      ...
      const res = await proveedoresService.getById(id);
      if (res.success) {
        setFormData({
        ...
        setError(res.error || 'No se pudo cargar el proveedor');
      }
    } catch (err) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Editar Proveedor</h1>
          <button
            onClick={handleCancel}
