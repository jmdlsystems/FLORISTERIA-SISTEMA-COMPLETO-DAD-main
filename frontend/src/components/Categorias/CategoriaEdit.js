...
const CategoriaEdit = () => {
  ...
  useEffect(() => {
    const fetchCategoria = async () => {
      const res = await categoriasService.getById(id);
      if (res.success) {
                ...
                <div className="flex items-center space-x-4">
                  <img
                    src={imagenFile
                      ...
                      : `https://grupo01tdam.pythonanywhere.com/assets/categorias/${imagen}`}
                    alt={nombre}
                    className="h-20 w-20 object-cover rounded border"
                    style={{ maxWidth: 80, maxHeight: 80 }}
                  />
                  <span className="text-sm text-gray-600">
                    {imagenFile ? 'Nueva imagen seleccionada' : 'Imagen actual'}
                  </span>
                </div>
              ) : (
                <div className="text-sm text-gray-500 italic">Sin imagen</div>
              )}
            </div>
