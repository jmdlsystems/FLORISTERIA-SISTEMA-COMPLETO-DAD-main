...
const OrdenCompraEdit = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const [ordenData, proveedoresData] = await Promise.all([
        ordenCompraService.getById(id),
        proveedoresService.getAll()
      ]);
      const orden = ordenData.data || ordenData;
      setFormData({
        numero: orden.numero || '',
        proveedorId: orden.proveedor_id ? String(orden.proveedor_id) : '',
        fecha: orden.fecha ? orden.fecha.split('T')[0] : new Date().toISOString().split('T')[0],
        estado: orden.estado || 'Pendiente',
        total: orden.total !== undefined && orden.total !== null ? orden.total : '',
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Editar Orden de Compra</h1>
          <button
            onClick={handleCancel}
