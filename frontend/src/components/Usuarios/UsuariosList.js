...
const UsuariosList = () => {
  ...
  useEffect(() => {
    fetchUsuarios();
  }, [location.pathname]); // Refrescar cuando cambie la ruta
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link
          to="/sistemafloreria/usuarios/crear"
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentUsuarios.map(usuario => (
              <tr key={usuario.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{usuario.rol?.nombre || ''}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div className="flex space-x-2">
                    <Link
                      to={`/sistemafloreria/usuarios/${usuario.id}/actualizar`}
      ...
      <div className="mt-4 text-sm text-gray-600">
        Mostrando {indexOfFirstUsuario + 1} a {Math.min(indexOfLastUsuario, filteredUsuarios.length)} de {filteredUsuarios.length} usuarios
      </div>
    </div>
  );
};
